...
class ImportEnvSlabsFromExcel extends Command
{
    protected $signature = 'mmc:import-env-slabs
        {--path= : Path to ireland_generic_mmc_model.xlsx}
        {--sheet="Slab Systems" : Sheet name}
        {--dataset-version=v2025.09 : Dataset version label}';

    protected $description = 'Import slab systems (supports webs, mass per 5.76 m², etc.). Aggregates A1–A3 per m².';
    ...
    public function handle(): int
    {
        ...
        $col = [
            ...
            'functional_role'  => $idx(['functional role']),
            'generic_material' => $idx(['generic material']),
            'width_m'          => $idx(['width (m)','width m']),
            'length_m'         => $idx(['length (m)','length m']),
            'thickness_m'      => $idx(['thickness (m)','thickness m']),
            'element_number'   => $idx(['element number','qty']),
            'total_volume_m3'  => $idx(['total volume (m3)','total volume']),
            'density_kg_m3'    => $idx(['density (kg.m3)','density (kg/m3)','density']),
            'mass_kg_per_5_76' => $idx(['mass (kg / 5.76 m²)','mass (kg/5.76 m2)']),
            'carbon_factor'    => $idx(['carbon factor']),
            'a1a3_per_5_76'    => $idx(['a1–a3 (kgco₂e / 5.76 m²)','a1-a3 (kgco2e / 5.76 m2)']),
            'a1a3_per_m2'      => $idx(['a1–a3 (kgco₂e/m²)','a1-a3 (kgco2e/m2)']),
        ];
        ...
        DB::transaction(function () use ($rows, $col, $ds, &$importedLayers, &$sumByCode) {
            for ($r=2; $r<=count($rows); $r++) {
                ...
                // Compute mass per m² if only mass per 5.76 m² exists
                $mass_kg_m2 = null;
                $mass_kg_5_76 = $this->float($row, $col['mass_kg_per_5_76']);
                if ($mass_kg_5_76 !== null) $mass_kg_m2 = $mass_kg_5_76 / 5.76;
                ...
                EnvironmentalLayer::updateOrCreate(
                    ...
                    [
                        ...
                        'width_m'=>$this->float($row, $col['width_m']),
                        'length_m'=>$this->float($row, $col['length_m']),
                        'thickness_m'=>$this->float($row, $col['thickness_m']),
                        'element_number'=>$this->intval($row, $col['element_number']),
                        ...
                        'a1a3_per_5_76'=>$this->float($row, $col['a1a3_per_5_76']),
                        'a1a3_per_m2'=>$this->float($row, $col['a1a3_per_m2']),
                    ]
                );
                $importedLayers++;

                $perM2 = $this->float($row, $col['a1a3_per_m2']);
                if ($perM2 === null) {
                    $mass = $mass_kg_m2;
        ...
        $this->info("Imported {$importedLayers} slab layer rows; updated ".count($sumByCode)." system A1–A3/m² entries (dataset {$ds->version_label}).");
        return self::SUCCESS;
    }
